...
def main():
    ...
    ap.add_argument('input', nargs = '?', default = None, 
                    help = "non-negative integer to be read")
    ...
    ap.add_argument("-R", "--read", 
        default = None,               # refreshed below upon reading pragmas
        help = "Read from standard input instead: int (default) or"\
    ...
    if (f := app.filename) is not None:
        ...
        if run:
            ...
            cp.ensure.that(
                app.input is not None or
                app.read in ("int", "intpair", "intseq", "nothing"), 
                f"No input but unknown --read value {app.read}")
            cp.ensure.that(
            ...
            if app.input is not None:
                print(outf(mainf(int(app.input))))
            else:
                match app.read:
                    case "int": 
